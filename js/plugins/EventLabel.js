...
(() => {
  ...
  PluginManager.registerCommand(
    PluginManagerEx.findPluginName(script),
    "SHOW_LABEL",
    function (args) {
      const id =
        PluginManagerEx.convertVariables(args.eventId) || this.eventId();
      const event = $gameMap.event(id);
      if (event) {
        event.setEventLabel(
          args.text,
          PluginManagerEx.convertVariables(args.fontSize)
        );
      }
    }
  );
  ...
  Game_Event.prototype.initialize = function () {
    ...
    this._labelSwitch =
      PluginManagerEx.findMetaValue(this.event(), "LB_S") || null;
  };
  ...
  Game_Event.prototype.isValidEventLabel = function () {
    return (
      this._labelText &&
      !this._erased &&
      !this.isHideLabelBecauseOfNoImage() &&
