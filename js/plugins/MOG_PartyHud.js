...
Moghunter.partyHud_SkipLeader = String(
  Moghunter.parameters["Skip Leader"] || "true"
);
Moghunter.partyHud_StartVisible = String(
  Moghunter.parameters["Start Visible"] || "true"
);
Moghunter.partyHud_SmartFade = String(
  Moghunter.parameters["Smart Fade"] || "true"
);
Moghunter.partyHud_NumberFontSize = Number(
  Moghunter.parameters["Font Size"] || 14
);
Moghunter.partyHud_FaceV = String(
...
Moghunter.partyHud_HPMeterV = String(
...
Moghunter.partyHud_MPMeterV = String(
...
Moghunter.partyHud_TPMeterV = String(
...
Moghunter.partyHud_HPNumberV = String(
  Moghunter.parameters["HP Number Visible"] || "true"
);
Moghunter.partyHud_HPNumberX = Number(
  Moghunter.parameters["HP Number X"] || 90
);
Moghunter.partyHud_HPNumberY = Number(
  Moghunter.parameters["HP Number Y"] || 12
);
Moghunter.partyHud_HPNumberA = Number(
  Moghunter.parameters["HP Number Align"] || 2
);
Moghunter.partyHud_MPNumberV = String(
  Moghunter.parameters["MP Number Visible"] || "true"
);
Moghunter.partyHud_MPNumberX = Number(
  Moghunter.parameters["MP Number X"] || 90
);
Moghunter.partyHud_MPNumberY = Number(
  Moghunter.parameters["MP Number Y"] || 36
);
Moghunter.partyHud_MPNumberA = Number(
  Moghunter.parameters["MP Number Align"] || 2
);
Moghunter.partyHud_TPNumberV = String(
  Moghunter.parameters["TP Number Visible"] || "true"
);
Moghunter.partyHud_TPNumberX = Number(
  Moghunter.parameters["TP Number X"] || 90
);
Moghunter.partyHud_TPNumberY = Number(
  Moghunter.parameters["TP Number Y"] || 66
);
Moghunter.partyHud_TPNumberA = Number(
  Moghunter.parameters["TP Number Align"] || 2
);
Moghunter.partyHud_LVNumberV = String(
  Moghunter.parameters["LV Number Visible"] || "true"
);
Moghunter.partyHud_LVNumberX = Number(
  Moghunter.parameters["LV Number X"] || 30
);
Moghunter.partyHud_LVNumberY = Number(Moghunter.parameters["LV Number Y"] || 4);
Moghunter.partyHud_LVNumberA = Number(
  Moghunter.parameters["LV Number Align"] || 1
);
Moghunter.partyHud_NameV = String(
...
Moghunter.partyHud_StatesV = String(
...
Moghunter.partyHud_ExpMeterV = String(
  Moghunter.parameters["Exp Visible"] || "true"
);
...
Moghunter.partyHud_Visible = [];
Moghunter.partyHud_LayX = [];
Moghunter.partyHud_LayY = [];
for (var i = 0; i < Moghunter.partyHud_Max; i++) {
  Moghunter.partyHud_LayX[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Position X"] || 0
  );
  Moghunter.partyHud_LayY[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Position Y"] || 0
  );
}
...
//==============================
var _mog_partyHud_scnMap_snapForBattleBackground =
...
PartyHud.prototype.createSprites = function () {
  this._smartFade =
    String(Moghunter.partyHud_SmartFade) === "true" ? true : false;
  ...
  this.createLayout();
  if (String(Moghunter.partyHud_FaceV) === "true") {
    this.createFace();
  }
  if (String(Moghunter.partyHud_HPMeterV) === "true") {
    this.createHPMeter();
  }
  if (String(Moghunter.partyHud_MPMeterV) === "true") {
    this.createMPMeter();
  }
  if (String(Moghunter.partyHud_TPMeterV) === "true") {
    this.createTPMeter();
  }
  if (String(Moghunter.partyHud_ExpMeterV) === "true") {
    this.createExpMeter();
  }
  if (String(Moghunter.partyHud_HPNumberV) === "true") {
    this.createHPNumber();
  }
  if (String(Moghunter.partyHud_MPNumberV) === "true") {
    this.createMPNumber();
  }
  if (String(Moghunter.partyHud_TPNumberV) === "true") {
    this.createTPNumber();
  }
  if (String(Moghunter.partyHud_LVNumberV) === "true") {
    this.createLVNumber();
  }
  if (String(Moghunter.partyHud_StatesV) === "true") {
    this.createStates();
  }
  if (String(Moghunter.partyHud_NameV) === "true") {
    this.createName();
  }
  if (this.needHide()) {
    this.opacity = 0;
...
PartyHud.prototype.refreshText = function (sprite, value, align) {
  ...
  sprite.bitmap.drawText(
    ...
    0,
    0,
    sprite.bitmap.width - 10,
    sprite.bitmap.height - 10,
...
PartyHud.prototype.needRefreshStates = function () {
  if (this._actor._needRefStatesHud) {
    return true;
  }
...
PartyHud.prototype.refreshHud = function () {
  if (this._hpmeter) {
    this.refreshMeter(this._hpmeter, this._actor.hp, this._actor.mhp);
  }
  if (this._mpmeter) {
    this.refreshMeter(this._mpmeter, this._actor.mp, this._actor.mmp);
  }
  if (this._tpmeter) {
    this.refreshMeter(this._tpmeter, this._actor.tp, 100);
  }
  if (this._expmeter) {
    if (this._actor.isMaxLevel()) {
      ...
      this.refreshMeter(
        this._expmeter,
        this._actor.current_exp_r(),
        this._actor.nextLevelExp_r()
      );
    }
  }
  if (this._hpnumber) {
  ...
  if (this._lvnumber) {
    this.refreshText(
      this._lvnumber,
      this._actor.level,
      Moghunter.partyHud_LVNumberA
    );
  }
  if (this._actorId != this._actor._actorId) {
    if (this._name) {
      this.refreshText(this._name, this._actor._name, Moghunter.partyHud_NameA);
    }
    if (this._face) {
      this.refreshFace();
    }
...
PartyHud.prototype.needRefreshHud = function () {
  ...
  if (this._hp[0] != this._actor.hp) {
    return true;
  }
  if (this._hp[1] != this._actor.mhp) {
    return true;
  }
  if (this._mp[0] != this._actor.mp) {
    return true;
  }
  if (this._mp[1] != this._actor.mmp) {
    return true;
  }
  if (this._lv != this._actor.level) {
    return true;
  }
  if (this._actorId != this._actor._actorId) {
    return true;
...
PartyHud.prototype.updateSprites = function () {
  ...
  if (this._hud_size[0] === -1 && this._layout.bitmap.isReady()) {
    this.getData();
  }
  if (this._state_icon) {
    this.updateStates();
