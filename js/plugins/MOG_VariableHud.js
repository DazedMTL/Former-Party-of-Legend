...
Moghunter.variableHud_smartFade = String(
  Moghunter.parameters["Smart Fade"] || "false"
);
Moghunter.variableHud_FieldVisible = String(
  Moghunter.parameters["Initial Visible"] || "false"
);
Moghunter.variableHud_Max = 20;
Moghunter.variableHud_Visible = [];
Moghunter.variableHud_VisibleInt = [];
Moghunter.variableHud_VariableID = [];
Moghunter.variableHud_ValueLimit = [];
Moghunter.variableHud_FileName = [];
Moghunter.variableHud_LayX = [];
Moghunter.variableHud_LayY = [];
Moghunter.variableHud_NumX = [];
Moghunter.variableHud_NumY = [];
Moghunter.variableHud_NumAlign = [];
Moghunter.variableHud_NumFontSize = [];
Moghunter.variableHud_Type = [];
Moghunter.variableHud_ShowMax = [];
Moghunter.variableHud_ShowGauge = [];
Moghunter.variableHud_gaugeX = [];
Moghunter.variableHud_gaugeY = [];
Moghunter.variableHud_AutoFade = [];
for (var i = 0; i < Moghunter.variableHud_Max; i++) {
  Moghunter.variableHud_Visible[i] = String(
    Moghunter.parameters["Hud " + String(i + 1) + " Enable"] || "false"
  );
  Moghunter.variableHud_VisibleInt[i] = String(
    Moghunter.parameters["Hud " + String(i + 1) + " Start Visible"] || "true"
  );
  Moghunter.variableHud_Type[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Mode"] || 0
  );
  Moghunter.variableHud_VariableID[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Variable ID"] || 1
  );
  Moghunter.variableHud_ValueLimit[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Maximum Value"] || 999
  );
  Moghunter.variableHud_FileName[i] = String(
    Moghunter.parameters["Hud " + String(i + 1) + " File Name"] || "Hud_"
  );
  Moghunter.variableHud_LayX[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Layout X"] || 0
  );
  Moghunter.variableHud_LayY[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Layout Y"] || 0
  );
  Moghunter.variableHud_NumX[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Number X"] || -7
  );
  Moghunter.variableHud_NumY[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Number Y"] || 10
  );
  Moghunter.variableHud_NumAlign[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Number Align"] || 1
  );
  Moghunter.variableHud_NumFontSize[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Font Size"] || 18
  );
  Moghunter.variableHud_ShowMax[i] = String(
    Moghunter.parameters["Hud " + String(i + 1) + " Show Max Value"] || "true"
  );
  Moghunter.variableHud_ShowGauge[i] = String(
    Moghunter.parameters["Hud " + String(i + 1) + " Show Gauge"] || "false"
  );
  Moghunter.variableHud_gaugeX[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Gauge X-Axis"] || 0
  );
  Moghunter.variableHud_gaugeY[i] = Number(
    Moghunter.parameters["Hud " + String(i + 1) + " Gauge Y-Axis"] || 0
  );
  Moghunter.variableHud_AutoFade[i] = String(
    Moghunter.parameters["Auto Fade"] || "true"
  );
}
...
Game_System.prototype.initialize = function () {
  ...
  this._variableHudFieldVisisble =
    String(Moghunter.variableHud_FieldVisible) === "true" ? true : false;
};
...
//==============================
var _mog_variableHud_scnMap_snapForBattleBackground =
...
Scene_Map.prototype.update = function () {
  ...
  if (this._variableHudField) {
    this._variableHudField.visible = $gameSystem._variableHudFieldVisisble;
  }
};
...
Scene_Map.prototype.createVariableHuds = function () {
  ...
  for (var i = 0; i < Moghunter.variableHud_Max; i++) {
    if (!$gameSystem._variableHudData[i]) {
      ...
      var vis =
        String(Moghunter.variableHud_VisibleInt[i]) === "true" ? true : false;
      $gameSystem._variableHudData[i].id = i;
      $gameSystem._variableHudData[i].visible = vis;
      $gameSystem._variableHudData[i].maxValue = Number(
        Moghunter.variableHud_ValueLimit[i]
      );
      $gameSystem._variableHudData[i].showMax =
        String(Moghunter.variableHud_ShowMax[i]) == "true" ? true : false;
      $gameSystem._variableHudData[i].gauge =
        String(Moghunter.variableHud_ShowGauge[i]) == "true" ? true : false;
      $gameSystem._variableHudData[i].gaugeX = Number(
        Moghunter.variableHud_gaugeX[i]
      );
      $gameSystem._variableHudData[i].gaugeY = Number(
        Moghunter.variableHud_gaugeY[i]
      );
      $gameSystem._variableHudData[i].autoFade =
        String(Moghunter.variableHud_AutoFade[i]) == "true" ? true : false;
    }
...
VariableHud.prototype.initialize = function (index) {
  ...
  this._index = index;
  this._enabled =
    String(Moghunter.variableHud_Visible[this._index]) === "true"
      ? true
      : false;
  this._smartFade =
    String(Moghunter.variableHud_smartFade) === "true" ? true : false;
  if (this._enabled) {
    this.createSprites();
  }
  this.visible = false;
...
VariableHud.prototype.number = function () {
  return this.type() === 0
    ? $gameVariables.value(this.variableID())
    : $gameParty.numItems(this.item());
};
...
VariableHud.prototype.refreshNumber = function () {
  ...
  var text = this.showMax()
    ? this._variable.padZero(maxv2) + "/" + this.maxValue()
    : String(this._variable);
...
VariableHud.prototype.needRefreshHud = function () {
  if (this._variable != this.number()) {
    return true;
  }
  if (this._maxValue != this.maxValue()) {
    return true;
  }
  return false;
};
...
VariableHud.prototype.needHide = function () {
  if (
    $gameMessage.isBusy() &&
    $gameSystem._variableHudData[this._index].autoFade
  ) {
    return true;
  }
  if (
    $gameSystem._variableHudData[this._index] &&
    !$gameSystem._variableHudData[this._index].visible
  ) {
    return true;
  }
