...
(function () {
  ...
  Game_Troop.prototype.setupDynamicCommon =
    Game_Map.prototype.setupDynamicCommon;
  Game_Troop.prototype.setupDynamicInterpreter =
    Game_Map.prototype.setupDynamicInterpreter;
  ...
  Sprite_Picture.prototype.update = function () {
    ...
    if (
      this.picture() &&
      this._touch &&
      !$gameSwitches.value(param.InvalidSwitchId)
    ) {
      this._touch.update();
    }
  };
  ...
  class PictureTouch {
    constructor(picture, id) {
      ...
      this._handlers[10] = () =>
        this.isTouchEvent(TouchInput.isDoubleTriggered);
    ...
    updatePictureTouch() {
      ...
      this._handlers.forEach((handler, trigger) => {
        if (this.isValidTrigger(handler, trigger)) {
          const eventData = $gameScreen.findPictureEvent(
            this._pictureId,
            trigger
          );
    ...
    updateFlick() {
      ...
      if (
        data.distance >= param.FlickDistance &&
        this.isOnPicturePos(data.start.x, data.start.y)
      ) {
        this._flick = true;
    ...
    isValidTrigger(handler, trigger) {
      ...
      return (
        eventData &&
        !$gameSwitches.value(eventData.invalidSwitchId) &&
        handler &&
        handler.call(this)
      );
    }
    ...
    isOnPicturePos(x = TouchInput.x, y = TouchInput.y) {
      ...
      const bx =
        Math.floor(dx * cos + dy * -sin) / pic.scale.x +
        pic.anchor.x * pic.width;
      const by =
        Math.floor(dx * sin + dy * cos) / pic.scale.y +
        pic.anchor.y * pic.height;
    ...
    isTouchPosInFrameWindow(x, y) {
      ...
      return (
        frame.x <= sx &&
        frame.x + frame.width >= sx &&
        frame.y <= sy &&
        frame.y + frame.height >= sy
      );
    }
