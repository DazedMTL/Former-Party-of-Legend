 ...
 * @param buyMessage
 * @text 購入画面メッセージ
 * @desc 購入画面で表示されるメッセージです。(ウィンドウ幅1/2)
 * ※設定なしでも可
 * @type multiline_string
 *
 * @param sellMessage
 * @text 売却画面メッセージ
 * @desc 売却画面で表示されるメッセージです。(ウィンドウ幅1/2)
 * ※設定なしでも可
 * @type multiline_string
 *
 * @param thanksMessage
 * @text お礼メッセージ
 * @desc 購入・売却完了時のメッセージです。(ウィンドウ幅1/2)
 * ※設定なしでも可
 * @type multiline_string
 *
...
(() => {
  ...
  const itemCategoryWindowDefault = eval(
    parameters["itemCategoryWindowDefault"] || "false"
  );
  let shopLists = [];
  if (shopListArrays) {
  ...
  Scene_Shop.prototype.loadShopExSettings = function () {
    shopSettings = shopLists.find(function (item) {
      if (Number(item.number) == $gameVariables.value(shopNumberVariable))
        return true;
    });
    if (!shopSettings) {
      shopSettings = {
  ...
  Scene_Shop.prototype.createStandingPicture = function () {
    ...
    this._staffPicture.bitmap = ImageManager.loadPicture(
      String(shopSettings.staffImageName)
    );
    this._staffPicture.x = Number(shopSettings.x);
    this._staffPicture.y = Number(shopSettings.y);
    this.addChildAt(
      this._staffPicture,
      this.children.indexOf(this._windowLayer)
    );
    // 対象アクター立ち絵
  ...
  Scene_Shop.prototype.createBackground = function () {
    if (shopSettings.backgroundImage) {
      ...
      this._backgroundSprite.bitmap = ImageManager.loadPicture(
        String(shopSettings.backgroundImage)
      );
  ...
  const _Scene_Shop_categoryWindowRect =
    Scene_Shop.prototype.categoryWindowRect;
        ...
        if (equipPicture) {
          ...
          if (
            ExMenuScreenBase.getHpRate(actor._actorId) >
              Number(equipPicture.pinchPercentage) ||
            !equipPicture.pinchImageName
          ) {
            // 通常
            this._actorPicture.bitmap = ImageManager.loadPicture(
              equipPicture.imageName
            );
          } else {
            // ピンチ
            this._actorPicture.bitmap = ImageManager.loadPicture(
  ...
  Scene_Shop.prototype.updateMessageWithPicture = function (messageType) {
    ...
    switch (messageType) {
      case "command":
        ...
        this._messageWindow.x =
          (Graphics.boxWidth - this._messageWindow.width) / 2;
      ...
      case "buy":
        ...
        this._messageWindow.y =
          Graphics.boxHeight - this._messageWindow.height - 148;
        this._messageWindow._text
          ? this._messageWindow.show()
          : this._messageWindow.hide();
        this._staffPicture.bitmap = ImageManager.loadPicture(
          String(shopSettings.staffImageName)
      ...
      case "thanks":
        ...
        if (shopSettings.staffThanksImageName) {
          this._staffPicture.bitmap = ImageManager.loadPicture(
            String(shopSettings.staffThanksImageName)
          );
        }
        break;
    }
  ...
  Scene_Shop.prototype.reserveStandingPictures = function () {
    if (shopSettings.staffImageName)
      ImageManager.loadPicture(String(shopSettings.staffImageName));
    if (shopSettings.staffThanksImageName)
      ImageManager.loadPicture(String(shopSettings.staffThanksImageName));
  ...
  Window_ShopItemCategory.prototype = Object.create(
    Window_ItemCategory.prototype
  );
  Window_ShopItemCategory.prototype.constructor = Window_ShopItemCategory;

  const _Window_ShopItemCategory_maxCols =
    Window_ShopItemCategory.prototype.maxCols;
